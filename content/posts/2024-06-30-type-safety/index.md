...
Type safety is all the rage these days. It’s the (one of the many)
...
**coercions** or your code may run as R intended—but not *as you*
intended.

<div class="aside">

💡 A type **coercion** is a type conversion that occurs because one type
does not match the other and is done silently. **Casting** is when you
explicitly change the type—e.g. calling `as.integer()` on `doubles()`
...
💡 A statically typed language requires you to specify the *type* of
...
the same exact fields, this wouldn’t work.
...
With this, though, you don’t actually need to have an instance of
...
and different), R doesn’t do any validation of arguments. TypeScript can
...
The standalone functions are quite unique. I’ve never seen anything
quite like them. They’re literally standalone R files with a bunch of
handy R functions. It’s like adding a package but without adding it as a
dependency.
...
Since this isn’t an R package, I will source the functions. Otherwise,
...
```
    ...
    [1] "/Users/josiahparry/github/misc/blog-zola/content/posts/2024-06-30-type-safety"
...
R doesn’t have the concept of a scalar. Though using a scalar is still
very useful in R.
...
In the case of `NULL`s we can provide the `allow_null` argument which
