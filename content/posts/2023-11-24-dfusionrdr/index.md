...
Want to skip to the end with the source code? [*Click
here*](#source-code).

## Goal

For this tutorial we’re going to create a very simple Rust-based R
package using [`extendr`](https://extendr.github.io/) and
[`arrow-extendr`](https://josiahparry.github.io/arrow-extendr/arrow_extendr/index.html).
The package will use the new and very powerful
[**`DataFusion`**](https://arrow.apache.org/datafusion/) crate to create
a csv reader.
...
We will use [`{usethis}`](https://usethis.r-lib.org/) to create a new R
package called `dfusionrdr` (prnounced d-fusion reader).

<div class="aside">

The following section is the standard process for creating a new Rust
based R package. It’s a pretty simple process once you get used to it!
...
do so we use`rextendr::use_extendr()`.
...
> `.Call()` function. You do not need to worry about this function. It’s
> just for context. :)
...
> `cmd + shift + b` or if on Windows it’s (probably) `ctrl + shift + b`.
...
We’ve now ran Rust code directly from R. Pretty simple Rust, but Rust
nonetheless.
...
> `Cargo.toml` you’ll now see these libraries added under the
...
1.  The `#[extendr]` macro indicates that this function will be made
    available to R.
2.  We add `/// @export` to indicate that our function will be exported
...
2.  This brings functions into the `NAMESPACE` and updates arguments and
    outputs
...
> **Warning**
>
> It’s typically a pretty bad idea to use `.unwrap()` since the program
...
[`tokio`](https://tokio.rs/) provides this for us with the `Runtime`
...
users](https://josiahparry.com/posts/2023-03-01-rust-traits-for-r-programmers/)
...
> **Tip**
>
> An `Robj` is `extendr`’s catch all for any type of object that can be
> returned to R
...
`Robj`. The [R
documentation](https://arrow.apache.org/docs/r/reference/RecordBatch-class.html)
...
`DataFrame`s have a method `.collect()` which creates a
`Vec<RecordBatch>`.
...
I’ve added a csv of
[`{palmerpenguins}`](https://allisonhorst.github.io/palmerpenguins/) to
...
from the
[`{nanoarrow}`](https://arrow.apache.org/nanoarrow/0.3.0/r/index.html) R
