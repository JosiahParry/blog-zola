---
title: 'Broadcasting: Scalars or vectors'
taxonomies:
  categories:
    - r
    - rlang
...
There’s a common pattern that we encounter when writing functions for R.
A single argument can often either be

- a scalar
- or a vector of the same length as another argument

When it’s a scalar, it makes sense to “broadcast” it to the same length
of another argument. Since R is vectorized we often want our functions
to be able to handle these scenarios.

## What is broadcasting?

Broadcasting definitely isn’t a new idea. It was first exposed to me
from [Kyle Barron’s](http://kylebarron.dev/) work in
[geoarrow-rs](https://docs.rs/geoarrow/0.4.0-beta.4/geoarrow/algorithm/broadcasting/enum.BroadcastablePrimitive.html).
It gave words to a pattern I have handled many times.

Broadcasting ensures that the “shape” of two arrays are the same. We are
...
- The [rray
  package](https://rray.r-lib.org/reference/rray_broadcast.html)

## My use case

In my work on the [R-ArcGIS
Bridge](https://developers.arcgis.com/r-bridge) we create many
[httr2](https://github.com/r-lib/httr2/) requests and send them in
parallel.

For ergonomic reasons, arguments should accept either a scalar OR a
vector of the same length. This is similar to R’s recycling
...
and broadcasting logic in every single function. That’s tedious and
error-prone.
...
I’ve proposed this for rlang in [issue
\#1819](https://github.com/r-lib/rlang/issues/1819). If you run into
